module MdIntegrations
  module Resources
    class Patients < Base
      ...
      def find(patient_id)
        connection.get("#{BASE_PATH}/#{patient_id}")
      end

      def search(filters = {})
        connection.post("#{BASE_PATH}/search", filters)
      end
      ...
      def dosespot_coverage(patient_id, params = {})
        connection.get("#{BASE_PATH}/#{patient_id}/dosespot/formulary", params)
      end
      ...
      def subscriptions(patient_id, params = { page: 1, per_page: 15 })
        connection.get("#{BASE_PATH}/#{patient_id}/subscriptions", params)
      end
